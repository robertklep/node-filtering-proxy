...
const request        = require('request').defaults({ followRedirect : false, encoding : null, timeout : 15000 });
...
const setFilterHeader = (res, msg) => res.set('x-filter', [ res.get('x-filter'), msg ].filter(x => x).join(', '));
      ...
      function modifyResponseMiddleware(req, res, next) {
        ...
        hijackResponse(res, function (err, res) {
          ...
          stream = stream.pipe(unstream((body, callback) => {
            ...
            // Update content-length (might have changed).
...
module.exports = singleton(options => new HttpProxy(options));
